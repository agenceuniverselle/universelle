...
const AddInvestmentDialog: React.FC<AddInvestmentDialogProps> = ({ 
  ...
  const onSubmit = (data: PropertyFormValues, publish = false) => {
    ...
    const propertyId = 'I' + Math.floor(Math.random() * 1000).toString().padStart(3, '0');
    
    const partnersArray = data.partners ? data.partners.split(',').map(p => p.trim()) : [];
    
    const documentUrls = documents.map(doc => URL.createObjectURL(doc));
    ...
    const newProperty = {
      ...
      isInvestment: true,
      investmentDetails: {
        type: data.investmentType as 'Résidentiel' | 'Commercial' | 'Touristique',
        returnRate: data.returnRate + '%',
        minEntryPrice: data.minEntryPrice + ' MAD',
        recommendedDuration: data.recommendedDuration,
        financingEligibility: data.financingEligibility,
        partners: partnersArray,
        projectStatus: data.projectStatus as 'Pré-commercialisation' | 'En cours' | 'Terminé',
    ...
    setTimeout(() => {
      ...
      if (publish) {
        ...
        toast({
          ...
          description: "Le bien d'investissement a été ajouté en mode brouillon et est visible dans la section Investissements.",
          variant: "default",
          action: (
            <Button 
              variant="default" 
