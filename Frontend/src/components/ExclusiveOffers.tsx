  ...
  <>
    <section id="exclusive" className="py-20 bg-luxe-blue text-white">
      <div className="container px-6 lg:px-10">
        ...
        <div
          key={offer.id}
          className="relative bg-white/5 backdrop-blur-sm rounded-xl overflow-hidden mb-10 transition-all duration-700 cursor-pointer"
          onClick={() => handleOpenInvestmentDetails(offer)}
        >
          {/* Flèches de navigation */}
          <button
            onClick={(e) => { e.stopPropagation(); setCurrentIndex((currentIndex - 1 + offers.length) % offers.length); }}
            className="absolute top-1/2 -left-0 transform -translate-y-1/2 z-10 text-white hover:text-gold text-4xl font-bold rounded-full px-3 py-1 transition"
            aria-label="Précédent"
          >
            ‹
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); setCurrentIndex((currentIndex + 1) % offers.length); }}
            className="absolute top-1/2 -right-0 transform -translate-y-1/2 z-10 text-white hover:text-gold text-4xl font-bold rounded-full px-3 py-1 transition"
            aria-label="Suivant"
          >
            ›
          </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2">
            {/* Image */}
            <div className="relative h-64 lg:h-auto hover:opacity-90 transition-opacity">
            ...
            <div className="p-8 lg:p-12">
              <h3 className="text-2xl font-bold mb-6 font-playfair hover:text-gold transition-colors">
