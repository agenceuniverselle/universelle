...
import { Search, TrendingUp, Briefcase, Luggage, Home, Filter, SlidersHorizontal } from 'lucide-react';
...
const Investir = () => {
 // const { properties } = useProperties();
  ...
  useEffect(() => {
    const fetchProperties = async () => {
        try {
            ...
            console.log(response.data); // Vérifiez ici les données reçues
  ...
  const filteredProperties = investmentProperties.filter(property => {
    ...
    const matchesReturn = returnFilter === 'all' ? true : 
      returnFilter === 'low' ? parseFloat((property.investmentDetails?.returnRate || property.return || '0%').replace('%', '')) <= 7 :
      returnFilter === 'medium' ? parseFloat((property.investmentDetails?.returnRate || property.return || '0%').replace('%', '')) > 7 && parseFloat((property.investmentDetails?.returnRate || property.return || '0%').replace('%', '')) <= 9 :
      parseFloat((property.investmentDetails?.returnRate || property.return || '0%').replace('%', '')) > 9;
    
    const matchesType = typeFilter === 'all' ? true : 
      property.investmentDetails?.type === typeFilter;
              ...
              <div className="flex gap-2">
                {typeFilter !== 'all' && (
                  <Badge variant="secondary" className="flex items-center gap-1">
                    {getInvestmentTypeIcon(typeFilter)}
                    {typeFilter}
                  </Badge>
                )}
                {returnFilter !== 'all' && (
                  <Badge variant="secondary" className="flex items-center gap-1">
                    ...
                    {returnFilter === 'low' ? 'Jusqu\'à 7%' : 
                     returnFilter === 'medium' ? '7-9%' : 
                     'Plus de 9%'}
                  </Badge>
                )}
                {statusFilter !== 'all' && (
